...
TEST_F(UpdateContextTestFixture, ScavengeFood)
{
	...
	settlement.GetLand(0)->GetNothing().AddOwner(std::unique_ptr<Person>(person1));

	Person* person2 = new Person(3000, 0, {});
	settlement.GetLand(0)->GetNothing().AddOwner(std::unique_ptr<Person>(person2));
	Food* food2 = new Food(100);
	settlement.GetLand(0)->GetNothing().GetNobody().Add(kFoodId, std::unique_ptr<Item>(food2));
